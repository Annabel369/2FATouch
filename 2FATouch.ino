...
void proximaTela() {
    ...
    int n = 6;
    int idx = 0;
    for (int i=0; i<n; i++) if(validos[i] == displayMode) { idx = i; break; }
...
void loop() {
  ...
  if (touchscreen.tirqTouched() && touchscreen.touched()) {
    if (millis() - last_tap > 500) {
      ...
      // z == 4095 significa que o SPI não conseguiu ler nada (desconectado ou conflito)
      // Um toque normal tem pressao entre 200 e 2000
      if (p.z > 100 && p.z < 3500) {
        proximaTela();
        last_tap = millis();
      }
  ...
  if (timeClient.isTimeSet()) {
      ...
      if (lastDay != -1 && currentDay != lastDay) {
          forceRedraw = true; // Força redesenho total da tela principal na virada da meia-noite
          lastDay = currentDay;
      } else if (lastDay == -1) {
          lastDay = currentDay;
      }
